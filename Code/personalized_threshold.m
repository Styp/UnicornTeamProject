function [BW,bounding_boxes] = personalized_threshold(original_image)
...
%blur the edges 
%BW_red = ordfilt2(BW_red,64,ones(8,8));
%BW_red = imfill(BW_red,'holes');

%BW_blue = ordfilt2(BW_blue,36,ones(6,6));
%BW_blue = imfill(BW_blue,'holes');

%BW_yellow = ordfilt2(BW_yellow,36,ones(6,6));
%BW_yellow = imfill(BW_yellow,'holes');

BW= BW_red + BW_blue + BW_yellow;

%detect the bounding boxes
test_image = bwlabel(BW);  % give a label to each connected component   

segments= regionprops(test_image, 'BoundingBox');
numberOfSegments = size(segments, 1);

bounding_boxes = zeros(numberOfSegments,4);
for k = 1:numberOfSegments
bounding_boxes(k,:) = segments(k).BoundingBox;
end
