function [ boundingBox, lable ] = detect_giveway_sign_angle (originalImage )
...
%number of different master angles
numMasterAngle = 10;
Angles = [0:0.05:0.2, 0.8:0.05:1];
...
houghSpace = zeros(imageSize(1),imageSize(2),numLayers,numMasterAngle);
...
for masterAngle = 1:1:numMasterAngle
    
    
    
%normalized angle    
gradientAtPoint = imageGradientMap(x(edgePoint), y(edgePoint))/pi();
gradientAtPoint = mod(gradientAtPoint + Angles(masterAngle) , 1);
...
[position(1), position(2),position(3),position(4)] = ind2sub(size(houghSpace),I);
